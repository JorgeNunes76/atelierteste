...
- Segurança / RLS: ⚠️ parcial (boas melhorias, mas há riscos pendentes em authz frontend e drift de `schema.sql`).
...
- `ProtectedRoute` aceita admin por `user_metadata.role`, além de `app_metadata.role` (vetor de elevação de privilégio se metadata insegura).
...
- Vulnerabilidades potenciais:
  - Admin check no frontend aceita `user_metadata.role` (alto risco de authz).
...
- Remover dependência de `user_metadata.role` no controlo admin (usar apenas fonte segura).
...
- Blocker técnico crítico identificado:
  - Autorização admin no frontend com `user_metadata.role` (alto risco).
...
1. Corrigir `ProtectedRoute` para confiar apenas em claims seguras (`app_metadata`/backend).
...
10. Congelar release com smoke test pós-deploy (checkout cartão + transferência + contacto).

## 13. Avaliação Final
- Estado atual: **pronto com reservas**.
- Justificação objetiva:
  - Fluxos críticos de pagamento/reserva/webhook estão tecnicamente robustos e bastante endurecidos.
  - Ainda existe um risco de segurança relevante no gate de admin e faltam testes automatizados críticos para garantir regressão contínua.
  - Sem esses dois pontos, o lançamento é possível mas com risco operacional acima do ideal para produção estável.
